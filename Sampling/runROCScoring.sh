...
do
	...
	then
		...
		# Extract probes
		cat "CountScores/"$protName"_ROC.tsv" | cut -f1 | sed "s/^/$lFlank/" | sed "s/$/$rFlank/" > probes.txt
		# Compute DB Scores
		...
		# Merge
		paste DBscores.txt NRLBscores.txt | sed "1s/.*/DeepBind$(printf '\t')NRLB/" > "CountScores/"$protName"_ROC_scores.tsv"
