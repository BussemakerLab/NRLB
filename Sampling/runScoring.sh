...
do
	...
	then
		...
		# Extract probes
		cat "CountScores/"$protName"_R0R1.tsv" | cut -f1 | sed "s/^/$lFlank/" | sed "s/$/$rFlank/" > probes.txt
		# Compute DB Scores
		./../DeepBind/deepbind/deepbind $dbID < probes.txt > DBscores.txt
		# Compute NRLB Scores
		...
		# Merge
		paste DBscores.txt NRLBscores.txt | sed "1s/.*/DeepBind$(printf '\t')NRLB/" > "CountScores/"$protName"_scores.tsv"
