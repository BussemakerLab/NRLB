...
For each available fly model, the corresponding genomewide, base-pair-resultion, affinity profile has been precomputed and is available on the Bussemaker Lab webserver (the plan is to add functionality to generate these genomewide affinity profiles within the `NRLBtools` package, but this require significant work, as the current scoring using `NRLBtools::score.genome` is too slow).

The precomputed affinity profiles can be fetched from the server in two different formats using `fetch.genomic.profiles` or `fetch.tracks`. Here, we will illustrate how to create bigwig files using the latter function.

The first step is to fetch the track, in the form of a `GRanges` object (this may   take several minutes when the `chr.names` option is omitted and profiles for all chromosomes are fetched):
```{r}
...
```
Note that `gr` contains relative affinity scores for multiple chromosomes and both strands:
```{r}
gr
```
...
```
To make the absolute NRLB score more meaningful, it should be properly normalized when constructing genomic tracks. The default behavior of `fetch.as.track` is to normalize the affinities by the genomidewide maximum. Alternatively, by setting `normalize = "model"` in `fetch.as.track`, we can achieve that the scores in `gr` are normalized relative to the theoretically optimal site. The optimal site may not occur in the genome, and therefore the genomewide maximum of `score(gr)` can be smaller than one with this normalization:
```{r}
range(score(gr))
```
It can be useful to save the track as a BigWig file for use in other applications. Note that the `gr` object above specifies the exact start and end of the binding site as an integer range. Overlapping ranges are not allowed in BigWig files, so we need to attribute the binding score for each window to a single genomic position by setting the width of the ranges to one:
```{r}
gr.new = gr
width(gr.new) = 1
gr.new
