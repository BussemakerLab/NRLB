...
df$seq = as.list(Views(BSgenome.Dmelanogaster.UCSC.dm3, 
                      GRanges(df$chr, IRanges(df$start, df$end), strand = "+")))
enhancers = apply(df, 1, as.list)
names(enhancers)
enhancers$E3N
```
...
```{r, fig.show='asis'}
model = hoxmodels$ExdUbx
nrlb.plot.score.genome(enhancers$E3N$seq, model$fits, model$index, model$mode, 
